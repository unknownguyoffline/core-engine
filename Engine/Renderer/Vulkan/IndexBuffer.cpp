...
void IndexBuffer::Create(VkDeviceSize capacity)
{
    mCapacity = capacity;
    ...
    mHandle = createBuffer(getDevice(), VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, mCapacity);
...
void IndexBuffer::StageData(size_t size, void *data)
{
    memcpy(mStageMemoryMap, data, size);
}

void IndexBuffer::PushData(VkDeviceSize size)
{
...
void IndexBuffer::SetData(VkDeviceSize size, void *data)
{
    StageData(size, data);
    PushData(size);
}
